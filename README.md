...
- **Selección de modelo por tarea y por tenant** (`GET /api/tenants/{tenantId}/ai-models/{providerName}`, `PUT|DELETE .../{task}`): un modelo barato para clasificar y uno bueno para redactar, con la posibilidad de que cada organización sobreescriba el suyo. Resuelve en dos capas (preferencia del tenant → configuración), con caché en memoria de un minuto e invalidación al escribir, para no pagar una consulta por llamada al modelo. El dato para medir el ahorro ya está en `ai_runs` (modelo y costo por llamada); el panel que lo muestra es `ORB-D11`. Elegir modelos es **solo del dueño**, a diferencia del resto de la configuración del asistente: cambia lo que se le cobra a la organización. `GET /api/ai-providers` lista los proveedores configurados, en el orden en que se intentan, para que la pantalla sepa qué nombre usar en la ruta. Es la implementación de `ORB-C13`.
